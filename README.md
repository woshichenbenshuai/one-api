...
> [!WARNING]
> 首次启动前请通过环境变量设置 `INITIAL_ROOT_USERNAME` 和 `INITIAL_ROOT_PASSWORD`，系统不再创建固定默认 root 口令。
...
```shell
# 使用 SQLite 的部署命令：
docker run --name one-api -d --restart always -p 3000:3000 -e SESSION_SECRET="replace-with-a-long-random-string" -e INITIAL_ROOT_USERNAME=admin -e INITIAL_ROOT_PASSWORD="replace-with-a-strong-admin-password" -e TZ=Asia/Shanghai -v /home/ubuntu/data/one-api:/data justsong/one-api
# 使用 MySQL 的部署命令，在上面的基础上添加 `-e SQL_DSN="root:your-db-password@tcp(localhost:3306)/oneapi"`，请自行修改数据库连接参数，不清楚如何修改请参见下面环境变量一节。
# 例如：
docker run --name one-api -d --restart always -p 3000:3000 -e SESSION_SECRET="replace-with-a-long-random-string" -e INITIAL_ROOT_USERNAME=admin -e INITIAL_ROOT_PASSWORD="replace-with-a-strong-admin-password" -e SQL_DSN="root:your-db-password@tcp(localhost:3306)/oneapi" -e TZ=Asia/Shanghai -v /home/ubuntu/data/one-api:/data justsong/one-api
```
...
初始化管理员账号由 `INITIAL_ROOT_USERNAME` 和 `INITIAL_ROOT_PASSWORD` 决定，可选配置 `INITIAL_ROOT_AUTHENTICATOR_SECRET` 以启用 TOTP。
...
> 正式部署请先复制 `.env.example` 为 `.env`，填好 `SESSION_SECRET`、数据库密码和初始管理员账号，再执行下面命令。

> 建议不要提交真实 `.env`，仓库中只保留 `.env.example` 作为模板。

> 当前仓库内的 `docker compose` 配置默认走本地构建，更适合 `arm64` 机器，也避免依赖远端预编译镜像是否提供对应架构。
...
2. 运行：
   ...
   ```
3. 访问 [http://localhost:3000/](http://localhost:3000/) 并登录。请使用 `INITIAL_ROOT_USERNAME` / `INITIAL_ROOT_PASSWORD` 创建的管理员账号登录。
...
2. `SESSION_SECRET`：设置之后将使用固定的会话密钥，这样系统重新启动后已登录用户的 cookie 将依旧有效。
   + 例子：`SESSION_SECRET=replace-with-a-long-random-string`
3. `SQL_DSN`：设置之后将使用指定数据库而非 SQLite，请使用 MySQL 或 PostgreSQL。
   + 例子：
     + MySQL：`SQL_DSN=root:your-db-password@tcp(localhost:3306)/oneapi`
...
本项目源码采用 MIT 协议，具体条款见仓库根目录 `LICENSE`。
